...
function Field({ label, name, value, onChange, readOnly = false }: any) {
  return (
    <div className="mb-5">
...
function Area({ label, name, value, onChange, rows = 4 }: any) {
  return (
    <div className="mb-5">
      <label className="mb-2 block text-sm font-semibold text-slate-700">{label}</label>
      <textarea
...
export default function AdminAboutPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (error || !data) {
          setMessage("about 데이터를 불러오지 못했습니다.")
          setLoading(false)
          return
        }
  ...
  const uploadMainImage = async (file: File) => {
    ...
    try {
      ...
      setMessage("대표 이미지 업로드 완료! 저장하기를 눌러주세요.")
    } catch {
  ...
  if (!authorized) return <div className="p-10">관리자만 접근할 수 있습니다.</div>
      ...
      <div className="mx-auto max-w-5xl rounded-3xl bg-white p-8 shadow-xl">
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-slate-500">/about 페이지 내용을 수정하세요.</p>
          </div>
          <button
        ...
        <Field label="메인 제목" name="hero_title" value={form.hero_title} onChange={handleInputChange} />
        <Area label="서브 문장" name="hero_subtitle" value={form.hero_subtitle} onChange={handleTextareaChange} />

        <h2 className="mb-4 mt-8 text-xl font-bold">Mission</h2>
        <Field label="배지 텍스트" name="mission_badge" value={form.mission_badge} onChange={handleInputChange} />
        <Field label="미션 제목" name="mission_title" value={form.mission_title} onChange={handleInputChange} />
        <Area label="미션 문단 1" name="mission_text_1" value={form.mission_text_1} onChange={handleTextareaChange} />
        <Area label="미션 문단 2" name="mission_text_2" value={form.mission_text_2} onChange={handleTextareaChange} />
        <Area label="미션 문단 3" name="mission_text_3" value={form.mission_text_3} onChange={handleTextareaChange} />
        ...
        <div className="mb-5">
          ...
          <label
            ...
            className={`flex min-h-[220px] cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed p-8 text-center transition ${
              dragOver ? "border-sky-500 bg-sky-100" : "border-sky-300 bg-sky-50 hover:bg-sky-100"
            }`}
          >
            ...
            <div className="mb-3 flex h-14 w-14 items-center justify-center rounded-full bg-white text-sky-600 shadow-sm">
              {uploadingImage ? <UploadCloud className="h-7 w-7 animate-pulse" /> : <ImagePlus className="h-7 w-7" />}
            </div>

            <p className="text-lg font-semibold text-sky-700">
              {uploadingImage ? "업로드 중..." : "이미지를 드래그하거나 클릭해서 업로드"}
            </p>
            <p className="mt-2 text-sm text-slate-500">PNG, JPG, JPEG, WEBP 파일 업로드 가능</p>
          </label>
        </div>
        ...
        <Field label="자료 수" name="archive_count" value={form.archive_count} onChange={handleInputChange} />
        <Field label="자료 수 라벨" name="archive_count_label" value={form.archive_count_label} onChange={handleInputChange} />

        <h2 className="mb-4 mt-10 text-xl font-bold">팀플 프로젝트 소개</h2>
